...
values ('aa@aa.com', 'Name1', '1111');
insert into member_table (member_email, member_name, member_password)
values ('bb@bb.com', 'Name2', '2222');
insert into member_table (member_email, member_name, member_password)       
values ('cc@cc.com', 'Name3', '3333');
-- 2. 이메일 중복체크 
...
select mem.custno as '회원번호', mem.custname as '회원성명', 
		case	when mem.grade = 'A' then 'VIP'
				when mem.grade = 'B' then '일반'
				when mem.grade = 'C' then '직원'
                else '없음'
		end as '고객등급',
        sum(mon.price) as '매출'
